-- ============================================================
-- GOLD LAYER VIEWS for HIVE CATALOG
-- Tạo views trong Hive Metastore để Trino và Metabase query
-- Catalog: hive (thay vì delta)
-- Schema: gold
-- ============================================================
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.daily_summary AS
SELECT 
...
FROM hive.gold.fact_transactions
GROUP BY 
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.hourly_summary AS
SELECT 
    YEAR(transaction_timestamp) as year,
    MONTH(transaction_timestamp) as month,
    DAY(transaction_timestamp) as day,
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.state_summary AS
SELECT 
...
FROM hive.gold.fact_transactions f
JOIN hive.gold.dim_customer c 
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.category_summary AS
SELECT 
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.amount_summary AS
SELECT 
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.latest_metrics AS
SELECT 
    -- Today's metrics
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.fraud_patterns AS
SELECT 
    amount_bin as amount_range,
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.merchant_analysis AS
SELECT 
    m.merchant,
    m.merchant_category,
...
-- ============================================================
CREATE OR REPLACE VIEW hive.gold.time_period_analysis AS
SELECT 
    t.time_period,
    t.is_weekend,
...
-- SELECT * FROM hive.gold.latest_metrics;

-- Example 2: Chatbot - "Which states have highest fraud?"
-- SELECT * FROM hive.gold.state_summary ORDER BY fraud_rate DESC LIMIT 10;

-- Example 3: Chatbot - "Show fraud patterns by amount"
-- SELECT * FROM hive.gold.fraud_patterns;

-- Example 4: Dashboard - Weekly trend
-- SELECT report_date, fraud_rate FROM hive.gold.daily_summary 
...
-- FROM hive.gold.fact_transactions f
-- JOIN hive.gold.dim_customer c ON f.customer_key = c.customer_key
-- JOIN hive.gold.dim_merchant m ON f.merchant_key = m.merchant_key
